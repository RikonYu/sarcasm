...
def read_pos(sent):
    ...
    ans=numpy.array([numpy.eye(pos_dict[i]+1)[pos_dict[i]] for _,i in ans])
    ...
    for i in range(toffset,len(tpos)):
        ...
        if(len(ins)>=batch_size and batch_size>0):
            if(singular==False):
                ...
                history=model.fit([x0,numpy.zeros(p0.shape),x1,numpy.zeros(p1.shape)],y,epochs=1,verbose=2,validation_split=0)
            fout.write(str(history.history['loss'][0]))
            fout.write('\n')
            ins=[]
            pos=[]
